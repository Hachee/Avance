//
//  CalendarioTableCell.h
//  TableViewAplication
//
//  Created by Héctor Omar Silva Díaz on 5/23/14.
//  Copyright (c) 2014 Hache Silva. All rights reserved.
//

#import <UIKit/UIKit.h>

@interface CalendarioTableCell : UITableViewCell

@property (strong, nonatomic) IBOutlet UILabel *tituloLabel;
@property (strong, nonatomic) IBOutlet UILabel *tituloLabel2;
@property (strong, nonatomic) IBOutlet UIImageView *thumbImagen;
@property (strong, nonatomic) IBOutlet UIImageView *thumbImagen2;

@end
